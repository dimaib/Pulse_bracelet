...
uint8_t MAX30100_Init(I2C_HandleTypeDef *ui2c, UART_HandleTypeDef *uuart){
	GPIOB->ODR&=~(1<<13);
	HAL_Delay(300);
	GPIOB->ODR|=(1<<13);
	_max30100_ui2c = ui2c;
	...
	HAL_Delay(100);
	if(MAX30100_ReadReg(MAX30100_REVISION)>0x00&&MAX30100_ReadReg(MAX30100_REVISION)<0xff){
		debug((uint8_t*)"pulse OK!\r\n",0);
		HAL_TIM_Base_Start_IT(&htim4);															//старт измерений пульса
		return 1;
	} else{ 
		debug((uint8_t*)"pulse ERROR!\r\n",0);
		return 0;
	}
}
...
float MAX30100_PlotTemperatureToUART(UART_HandleTypeDef *uuart){
	uint8_t tempInt = _max30100_temp / 1;
	uint8_t tempFrac = (_max30100_temp - tempInt) * 10;
	//char data[15];
	//sprintf(data, "%d.%d\n", tempInt, tempFrac);
	float tmp=(float)tempInt+(float)(tempFrac/10.0);
	//sprintf(data, "%.2f\n", tmp);
...
//uint16_t chanel_ir[BLOCK_SIZE]={0};
//uint16_t chanel_red[BLOCK_SIZE]={0};

float32_t chanel_ir[BLOCK_SIZE]={0};
float32_t chanel_red[BLOCK_SIZE]={0};
uint16_t incr_ir_red=0;

extern uint8_t math_start;

void MAX30100_PlotBothToUART(UART_HandleTypeDef *uuart, uint16_t *samplesRed, uint16_t *samplesIr, uint8_t sampleSize){
	char data[50];
	//if(math_start==0){
		for(uint8_t i = 0; i< sampleSize; i++){
				chanel_ir[incr_ir_red]=samplesIr[i];
				chanel_red[incr_ir_red]=samplesRed[i];
				incr_ir_red++;
							//sprintf(data,"hr:%d spo2:%d\r\n",samplesIr[i],samplesRed[i]);
							//debug((uint8_t*)data,0);
				if(incr_ir_red>=BLOCK_SIZE){
					incr_ir_red=0;
					math_start=1;
				}
		}
