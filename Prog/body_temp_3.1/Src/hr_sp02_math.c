...
void hr_math_(float32_t *testInputR, float32_t *testInputIR)
{
	...
	if(meanIR>20000&&meanR>20000){
		...
		rate_limiter_f32(&buf1[0], blockSize, 10, ratePrevR);
		...
		dead_zone_f32 (&buf1[0], blockSize, rmsIR/3, 0, &tailBufIR[0]);//remove too small peaks
		#if AVERAGING > 0
		...
		hr = num[0]*60*100/blockSize;//averaged number of maximums in window*60seconds*100Hz/window length
		printf("heart rate is %.5f, SpO2 is %.5f\r\n", hr, spo2);
		#endif
		
		Puls_serv=hr;
		if(spo2<96) 		SpO2=96;
		else if(spo2>99)SpO2=99;
