...
void hr_math_(float32_t *testInputR, float32_t *testInputIR)
{
	...
	if(meanIR>20000&&meanR>20000){
		...
		for(uint32_t j=0; j < TAIL; j++) tailBuf[j] = filteredSignal[blockSize - TAIL + j];
		...
		for(uint32_t j=0; j < TAIL; j++) tailBufIR[j] = filteredSignalIR[blockSize - TAIL + j];
		derivative_f32 (&filteredSignalIR[0], &buf1[0], blockSize, 1, derivPrev);
		arm_fir_f32(&LowPassDerivative, &buf1[0], &buf2[0], blockSize);
		for(uint32_t j=0; j < TAIL; j++) tail[j] = buf2[blockSize - TAIL + j];
		...
		dead_zone_f32 (&buf1[0], blockSize, 70, 0, &tailBufIR[0]);//39
		#if AVERAGING > 0
		uint32_t sum = 0;//sum of samples
		float32_t ls = 0;//local sum of SpO2
		...
		spo2 = ls/AV_FACTOR;
		hr = (float32_t)sum*6000/blockSize;
		...
		hr = (float32_t)num[0]*6000/blockSize;
		//printf("heart rate is %.5f, SpO2 is %.5f\r\n", hr, spo2);
		Puls_serv=hr;
		SpO2=spo2;
		#endif
